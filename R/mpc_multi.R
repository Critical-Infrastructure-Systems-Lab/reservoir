...
simcast_multi <- function(Q, forecast, start_yr, capacity, target, surface_area,
                          ...
                          vol_targ = 0.75, weights = c(0.7, 0.2, 0.1),
                          S_disc = 1000, R_disc = 10,
                          Q_disc = c(0, 0.2375, 0.4750, 0.7125, 0.95, 1),
                          loss_exp = c(2,2,2), S_initial = 1, plot = TRUE){
  ...
  x <- reservoir::sdp_multi(Q = Qtr, capacity = capacity, target = target,
  ...
  for(yr in 1:nrow(Q_month_mat)){
    for (month in 1:frq){
      ...
      R[t_index] <- reservoir::dp_multi(Q = Q_forecast, capacity = capacity,
                                        target = target, surface_area = surface_area,
                                        max_depth = max_depth, R_max = R_max,
                                        spill_targ = spill_targ, vol_targ = vol_targ,
                                        weights = weights, loss_exp = loss_exp, 
                                        evap = evap_forecast, S_disc = S_disc, R_disc = R_disc,
                                        S_initial = Storage / capacity, c2g = c2g,
                                        plot = FALSE)$releases[1]
  ...
  total_spill_cost <- sum((Sp / quantile(Q, spill_targ)) ^ loss_exp[2])
  ...
  total_release_cost_sdp <- sum((1 - xx$releases / target)[which((xx$releases / target) <  1)] ^ loss_exp[1])
  total_spill_cost_sdp <- sum((xx$spill / quantile(Q, spill_targ)) ^ loss_exp[2])
  total_volume_cost_sdp <- sum(((xx$storage - vol_targ * capacity) / (vol_targ * capacity)) ^ loss_exp[3])
  total_weighted_cost_sdp <- weights[1] * total_release_cost_sdp + weights[2] * total_spill_cost_sdp + weights[3] * total_volume_cost_sdp 
  ...
  results <- list(S, R, E, y, Sp, costs, cost_saving)
  names(results) <- c("storage", "releases", "evap_loss", "water_level",
                      "spill", "costs", "percent_cost_saving")
  return(results)
}