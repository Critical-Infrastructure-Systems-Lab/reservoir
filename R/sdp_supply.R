...
sdp_supply <- function (Q, capacity, target, surface_area, max_depth, evap,
  ...
  if (rep_rrv == TRUE){
    ...
    rrv_ <- rrv(R_rec, target)
    results <- list(R_policy, Bellman, S, R_rec, E, y, Spill, unname(rrv_)[1], unname(rrv_)[2],
                    unname(rrv_)[3], unname(rrv_)[4], unname(rrv_)[5], total_penalty)
    names(results) <- c("release_policy", "Bellman", "storage", "releases", "evap_loss",
                        "water_level", "spill", "annual_reliability",
